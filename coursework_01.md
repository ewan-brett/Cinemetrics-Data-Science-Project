...
*UP has the lowest outlier in ratings, roughly 20/100, meaning one user
gave it a very bad rating. However for overall audience opinion, we
discount this and look at the IQR, which is relatively small compared to
the others in top 10, hence greater consensus among user ratings.*
...
*This supports my interpretation: Up has the second lowest IQR of 19.0,
so less variation in opinions. Inside out is the film with least
consensus with IQR of 22.5, and dune has the greatest consensus with IQR
of 18.0.*
...
higher than that of basic users (44.0). However splitting our analysis
by age category, basic users have higher engagement than premium for all
...
of users. For instance, premium users are more concentrated in higher
age categories (as seen in verification), where engagement levels are
higher, hence the average engagement is weighted towards premium. On the
other end, 91.0% of users 25 or younger have basic subscription, but the
engagement in age tier 1 is significantly lower in general (28.9 for
basic and 12.6 for premium) hence engagement for basic users is a lot
lower, despite being higher in every individual age tier.*
...
3.  *The most frequent words in Romance are “film” (697), “watch” (680),
    “average” (359). These are generic words that don’t tell us about
    Romance genre specifically. The 3 highest TF-IDF words are “watch”
    (0.014), “average” (0.007), “fairly” (0.007). In this scenario, due
    only having 8 genres, and the language used across reviews for any
    film is quite similar, the IDF component does not distinguish terms
    as effectively. It does help slightly though, by removing “film”,
    the most generic term. Instead we see “fairly” which does give us
    more insight into the general strength of opinions about Romance
    films.*
...
*I would suggest that CineMetrics should focus on promoting content
specific to users. For example, engagement for users 25 or younger was
28.94 (basic) and 12.67 (premium), suggesting CineMetrics needs to focus
on promoting more interesting content to younger age groups. It could be
useful to include a wider variety of genres that appeal to children
more, as right now the best performing films in terms of rating is
Horror with 79.8, however this is not suitable for children. Further,
Inside out has a rating IQR of 22.5 meaning opinion was quite varied.
This could be because it is a childrens’ film which has been wrongly
recommended to adults, hence tailoring recommendations will reduce
variation in opinions. One limitation is the TF-IDF sentiment has
wrongly interpreted the opinion of horror movie reviews. To better
categorise this we should use an adapted version of bing to fit the
context of movies.*
